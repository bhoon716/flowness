...
Flowness 是一个面向可追踪 AI 代理开发的对话式工作流 harness (conversational workflow harness)。它把自然语言请求转换成受跟踪的问题单（Issue），通过显式工作流推进工作，保留 append-only 的证据与日志，并让结构化 review 检查和规则变更保持可追踪。
